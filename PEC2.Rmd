...
params:
  file1: peptidos_transf_one_hot.csv
  file2: peptidos.csv
  folder.data: dataset
  p.train: !r 2/3
...
- Clasificación, incluyendo el reconocimiento de patrones y la secuencia de reconocimiento, detección y de la toma de decisiones secuenciales (diagnostico m'edico, aplicaciones financieras, sistemas radar, reconocimiento facial, clasificación de señales, control del vehículo, predicción de trayectorias, el control de procesos, manejo de recursos naturales, juegos y la toma de decisiones como el backgammon, ajedrez, póquer...)
- Procesamiento de datos, incluyendo el filtrado, el agrupamiento, la separación ciega de las señales y compresión (diferenciando entre informes deseados y no deseados en redes sociales y prevención de spam)
...
| - No necesita muchas restricciones acerca de las relaciones subyacentes de los datos | - Es un modelo de caja negra complejo que es dificil, si no imposible, de interpretar
...
Hemos incluido varios parámetros en el encabezado `YAML` del documento cuyos valores se pueden establecer cuando se procesa el informe. Se ha incluido tanto la semilla que emplearemos más tarde en la creación de los datos de test y de entrenamiento así como los nombres de los archivos y la ruta de acceso, de esta forma podemos leer los datos con el siguiente código:

```{r datos,message=FALSE,warning=FALSE}
# Ahora ya se importan los datos a formato data.frame
library(readr)
peptidos <- read_delim(file=file.path(params$folder.data,params$file2),
...
Nuestro dataframe está compuesto por 15840 observaciones de 2 variables de caracteres, representando la primera de ellas la secuencia del peptido y la segunda el factor que nos indica si la secuencia peptídica interacciona o no con MHCI. Como vemos tenemos el mismo numero de sucencias que interaccionan como secuencias sin interacción.

Para representar estos datos, podemos emplear su secuencia logo (https://en.wikipedia.org/wiki/Sequence_logo) con la ayuda del paquete `ggseqlogo`. Un logotipo de secuencia consiste en una pila de letras en cada posición. Los tamaños relativos de las letras indican su frecuencia en las secuencias. La altura total de las letras representa el contenido de la información de la posición, en bits.

```{r secuencialogo, warning=FALSE,message=FALSE,tidy=TRUE,fig.height=4}
...
## Codificación "one-hot" (one-hot encoding) de las secuencias.
...
Empleando la función creada en el apartado anterior, transformamos las secuencias de aminoácidos en vectores numéricos y estos en un data frame que podamos emplear en los siguientes pasos, al que denominamos *dataset*.
...
dataset <- as.data.frame(t(as.data.frame(strsplit(data, "", 
              perl = TRUE))), col.names = v1, row.names = FALSE)
indx <- sapply(dataset, is.character)
dataset[indx] <- lapply(dataset[indx], function(x) as.numeric(as.character(x))) 

```

Creamos y guardamos un archivo csv para poder utilizarlo posteriormente:

```{r eval=FALSE, include=TRUE}
write.csv(dataset, "./dataset/peptidos_transf_one_hot1.csv", 
...
Podemos emplear directamente el archivo creado en los pasos anteriores *dataset* o cargar los datos desde la carpeta donde los hemos creado. Seguiremos este segundo paso, para no ir arrastrando errores en caso de que los haya:

```{r data, message=FALSE, warning=FALSE}
library(readr)

# Mis datos
data.nn <- read_csv("dataset/peptidos_transf_one_hot1.csv")
...
### Paso 3. Semilla aleatorio

```{r seed}
# set.seed(params$seed.clsfier) # para garantizar que
                                # los resultados sean reproducibles
```

### Paso 4. Modelos de red neuronal artificial de una sola capa oculta con 1 y 3 nodos

Para la construcción de la red neuronal artificial se usa la función `neuralnet()` del paquete *neuralnet*:

La fórmula del modelo tiene `r ncol(data.nn)` nodos de entrada y `nlevels(peptidos$label)` nodos de salida:
...
nn.model.1.matrix <- compute(nn.model.1, data.nn.test[,1:180])$net.result
...
El resultado de la matriz de confusión con los datos de test en el último modelo con tres nodos sería:

```{r Mconfusion3,warning=FALSE,message=FALSE,tidy=TRUE}
nn.model.3.matrix <- compute(nn.model.3, data.nn.test[,1:180])$net.result
...
Al comparar los dos modelos vemos que obtenemos resultados muy similares, por lo que escogemos el modelo más sencillo, de forma que podamos equilibrar la complejidad y la precisión del modelo. Los modelos más complejos también son más susceptibles de tener overfitting.

### Paso 6. Paquete caret modelo "mlp"

El paquete `caret` (abreviatura de Classification And REgression Training) contiene funciones para simplificar el proceso de entrenamiento de modelos para problemas complejos de regresión y clasificación. El paquete utiliza una cantidad de paquetes R [@kuhn2012caret] pero intenta no cargarlos todos al inicio del paquete (al eliminar las dependencias formales del paquete, el tiempo de inicio del paquete puede disminuir considerablemente).
...
Como vemos, con un modelo mlp con 3 nodos se consigue un accuracy de validación promedio del 99.8%.
La diferencia en precisión con los modelos anteriores de 1 y 3 nodos, pero empleando la otra función, no difiere mucho de los resultados obtenidos en este modelo. Es un modelo mucho más robusto que los anteriores, sin embargo, debido al tiempo de ejecución, quizá en este caso no es necesario, ya que con los modelos anteriores tenemos una buena "accuracy".
...
Empleamos las variables binarias de base de datos transformada con la variable categorica "label" como ultima variable, al igual que hemos realizado para el modelo de red neuronales con el paquete caret.
...
El porcentaje de muestras tumorales en los datos de training es de `r round((table(dataset.train$y)[1]/ nrow(dataset.train))*100,2)`%, y en los de test de `r round((table(dataset.test$y)[1]/ nrow(dataset.test))*100,2)`%. Si el tamaño de la muestra es bajo, podríamos emplear otros métodos como k-fold cross validation o bootstrap.

### Paso 3. semilla generadora el valor 1234567.

Para garantizar que los resultados sean reproducibles, incluimos la siguiente semilla, como tenemos el valor de la semilla incluida en el encabezado `YAML` del documento, solo debemos llamarla con el siguiente script, que incluiremos más adelante además cuando creemos el modelo.

```{r}
set.seed(params$seed.clsfier) # to guarantee repeatable results
```

### Paso 4. Utilizar la función lineal y la RBF para ajustar un modelo de SVM

El algoritmo de SVM que podemos emplear es la función `ksvm()` del paquete *kernlab*. Aunque hay otras opciones, como la funcion svm() del paquete `e1071` identifica automáticamente si se trata de un problema de clasificación, la variable respuesta es de tipo factor, o de regresión, la variable respuesta es tipo numérico. 
...
Ahora trataremos de mejorar el rendimiento de nuestro modelo implementando un SVM pero con un el kernel Gaussiano, `rbfdot`. Tambien se podría aplicar la funcion `tune` del paquete `e1701`.
...
Como vemos ambas tienen resultados muy similares, por lo que escogemos el modelo lineal que es mas sencillo. Debemos tratar de equilibrar la complejidad y la precisión del modelo, evitando el overfitting de los modelos complejos.

### Paso 6. Paquete caret modelo svmRBF con 5-fold crossvalidation

Emplamos los grupos de entrenamiento y test creados en el apartado anterior con la funcion *createDataPartition* para el paquete caret, ya que este paquete admite datos de tipo factor. 
...
Empleamos ahora el paquete `caret` con el modelo `svmRB`F para aplicar el algoritmo de SVM con 5-fold crossvalidation.
...
# Con la funcion "predict" también podemos obtener la probabilidad para cada clase:
prediction.rbf.fold5.prob <- predict(model.rbf.fold5, test.set[-181], type="prob")  
head(prediction.rbf.fold5.prob)
